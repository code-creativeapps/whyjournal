...
- todos: today-sized only. If a starter task doesn't belong to any project, set projectRef="" (the app handles standalone tasks). Do NOT invent fake projectRefs. ALWAYS include 1–3 setup/first-rep tasks even if there is no project — these are the concrete first actions that install the habits and break inertia. Examples: "Download Duolingo and complete Italian Lesson 1", "Book first iTalki trial lesson", "Pin Anki to home screen and import Italian top-1000 deck". A plan with habits but zero tasks is incomplete — the user needs something to do TODAY to start.
